...
def main(group, nhvg, K, topn, ldthresh=0.2):
    prefix = './'
    
    cnmf_obj = cNMF(output_dir=prefix+f'data/cNMF/', name=f'{group}_v{nhvg}')
    norm_hvg = sc.read(cnmf_obj.paths['normalized_counts']) ## Load the normalized data used for cNMF, in this case with metacells
    ...
    refit_usage.to_csv(prefix+f'data/{topn}/refit/cre_{group}_v{nhvg}_K{K}_usage.csv', index_label='barcode')
